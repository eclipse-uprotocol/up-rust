...
use crate::{
    InitializedStablePayload, NativePayloadIdentity, NativeProfileAgreement, NativeRepresentation,
    PayloadIdentity, StableContainerPayload, StablePayload, StablePayloadInit, UFrameMetadata,
    UUninitTxBuffer, UVecUninitTxBuffer, UWireError,
};
...
/// Describes a fixture's complete native layout without assigning a deployment ID.
pub fn stable_payload_representation(
    case: &PayloadContractCase,
) -> Result<NativeRepresentation, UWireError> {
    match case.kind {
        PayloadContractCaseKind::CanClassicMax => {
            StableContainerPayload::<CanClassicFrameV1>::validate_representation()
        }
        PayloadContractCaseKind::CanFdMax => {
            StableContainerPayload::<CanFdFrameV1>::validate_representation()
        }
        PayloadContractCaseKind::SomeIpSingleMtu => {
            StableContainerPayload::<SomeIpSignalBatchMtuV1>::validate_representation()
        }
        PayloadContractCaseKind::Streamer4k => {
            StableContainerPayload::<StreamChunk4kV1>::validate_representation()
        }
        PayloadContractCaseKind::RadarArs548DetectionList => {
            StableContainerPayload::<RadarDetectionListArs548V1>::validate_representation()
        }
        PayloadContractCaseKind::Streamer64k => {
            StableContainerPayload::<StreamChunk64kV1>::validate_representation()
        }
        #[cfg(feature = "payload-contract-large-fixtures")]
        PayloadContractCaseKind::LidarHesaiAt128PointCloud => {
            StableContainerPayload::<LidarPointCloudHesaiAt128V1>::validate_representation()
        }
        #[cfg(feature = "payload-contract-large-fixtures")]
        PayloadContractCaseKind::Camera8mpBayerRggb12p => {
            StableContainerPayload::<CameraBayerRggb12pFrame8mpV1>::validate_representation()
        }
        #[cfg(feature = "payload-contract-simulator-fixtures")]
...
pub struct StableOwnedFixture {
    pub identity: NativePayloadIdentity,
    pub bytes: Vec<u8>,
    pub stable_type_name: &'static str,
    pub stable_transport_len: usize,
    pub stable_align: usize,
...
pub fn stable_owned_fixture_for(
    ...
    match case.kind {
        PayloadContractCaseKind::CanClassicMax => {
            stable_owned_fixture_for_type::<CanClassicFrameV1>(profile, |storage| {
                init_can_classic_max(CanClassicFrameV1::init(storage)?, sequence).map(drop)
            })
        }
        PayloadContractCaseKind::CanFdMax => {
            stable_owned_fixture_for_type::<CanFdFrameV1>(profile, |storage| {
                init_can_fd_max(CanFdFrameV1::init(storage)?, sequence).map(drop)
            })
        }
        PayloadContractCaseKind::SomeIpSingleMtu => {
            stable_owned_fixture_for_type::<SomeIpSignalBatchMtuV1>(profile, |storage| {
                init_someip_single_mtu(SomeIpSignalBatchMtuV1::init(storage)?, sequence).map(drop)
            })
        }
        PayloadContractCaseKind::Streamer4k => {
            stable_owned_fixture_for_type::<StreamChunk4kV1>(profile, |storage| {
        ...
        PayloadContractCaseKind::Streamer64k => {
            stable_owned_fixture_for_type::<StreamChunk64kV1>(profile, |storage| {
                init_streamer_64k(StreamChunk64kV1::init(storage)?, sequence).map(drop)
            })
        }
        #[cfg(feature = "payload-contract-large-fixtures")]
        PayloadContractCaseKind::LidarHesaiAt128PointCloud => {
            stable_owned_fixture_for_type::<LidarPointCloudHesaiAt128V1>(profile, |storage| {
...
pub fn validate_stable_owned_bytes(
    ...
    let representation = stable_payload_representation(case)?;
    let identity = profile.identity_for(&representation)?;
    PayloadIdentity::Native(identity)
...
pub fn validate_stable_owned_bytes_exact(
    ...
    validate_stable_owned_bytes(case, sequence, metadata, profile, bytes)?;
    let expected = stable_owned_fixture_for(case, sequence, profile)?;
...
{
    let identity = StableContainerPayload::<T>::identity(profile)?;
    let metadata = fixture_metadata(identity);
    let mut buffer =
...
fn fixture_metadata(identity: NativePayloadIdentity) -> UFrameMetadata {
