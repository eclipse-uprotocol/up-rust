...
//! uProtocol TCK conformance rows for UUri behavior.

use std::str::FromStr;

use cucumber::{given, then, when};
use up_rust::{ProtobufMappable, UUri};
...
async fn serialize_to_uri(w: &mut UUriWorld) {
    w.uuri = Some(
        UUri::try_from_parts(
            w.authority_name.as_str(),
            w.ue_id,
            w.ue_version_major,
            w.resource_id,
        )
        .expect("failed to create UUri from parts"),
    );
    w.uri = w.uuri.as_ref().expect("UUri is not set").to_uri(true);
}
...
async fn assert_original_uuri_can_be_recreated_from_protobuf(w: &mut UUriWorld) {
    assert!(UUri::parse_from_protobuf_bytes(&w.protobuf).is_ok_and(|uuri| w.uuri.eq(&Some(uuri))));
}
...
async fn assert_uuri_can_be_deserialized_from_bytes(w: &mut UUriWorld, hex_string: String) {
    ...
    assert!(
        UUri::parse_from_protobuf_bytes(buf.as_slice()).is_ok_and(|uuri| w.uuri.eq(&Some(uuri)))
    );
}
...
async fn assert_original_uuri_can_be_recreated_from_uri_string(w: &mut UUriWorld) {
    assert!(w
        .uri
        .parse::<UUri>()
        .is_ok_and(|uuri| w.uuri.eq(&Some(uuri))));
}
...
async fn assert_uuri_matches_pattern(w: &mut UUriWorld, pattern: String) {
    assert!(pattern
        .parse::<UUri>()
        .is_ok_and(|p| p.matches(w.uuri.as_ref().expect("UUri is not set"))));
}
