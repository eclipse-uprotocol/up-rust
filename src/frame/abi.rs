...
///
/// Values are limited to 16 bits while this profile retains a four-byte slot.
/// Presence is determined by the field bit, not the numeric value: a present
/// zero means contract-defined encoding. Producers zero absent storage.
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct UPayloadEncodingAbi {
    /// Payload identifier (0..=65535), meaningful when the presence bit is set.
    pub registry_id: u32,
}
                ...
                Some(
                    PayloadEncoding::from_id(self.payload_encoding.registry_id).map_err(|_| {
                        UFrameAbiError::InvalidProfile(
                            "payload-encoding identifier exceeds 65535".to_string(),
                        )
                    })?,
                ),
...
mod tests {
    ...
    #[test_case::test_case(8; "unassigned identifier")]
    #[test_case::test_case(0xE000; "reserved identifier")]
    #[test_case::test_case(0xFFFF; "private maximum")]
    fn identifier_and_empty_presence_survive_fixed_profile(id: u32) {
        ...
        let metadata = UFrameMetadata::publish(topic())
            ...
            .unwrap();
        let abi = UFrameMetadataAbiV1::try_from_metadata(&metadata, Some(0)).unwrap();
        assert_ne!(abi.presence & FIELD_PAYLOAD_ENCODING, 0);
        assert_eq!(abi.payload_encoding.registry_id, id);
        let (decoded, size) = abi.try_to_metadata().unwrap();
        assert_eq!(decoded.payload_encoding(), Some(&encoding));
        assert_eq!(size, Some(0));
    }

    #[test_case::test_case(65536; "just above 16 bits")]
    #[test_case::test_case(u32::MAX; "maximum storage value")]
    fn oversized_identifier_is_rejected_without_truncation(id: u32) {
        let metadata = UFrameMetadata::publish(topic())
            .with_payload_encoding(PayloadEncoding::RAW)
            .build()
            .unwrap();
        let mut abi = UFrameMetadataAbiV1::try_from_metadata(&metadata, Some(0)).unwrap();
        abi.payload_encoding.registry_id = id;
        assert!(abi.try_to_metadata().is_err());
    }
